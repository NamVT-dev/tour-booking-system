...
const getAllUserForAdmin = catchAsync(async (req,res,next) => {
...
const createPartnerAccount = catchAsync(async (req, res, next) =>{
    ...
    console.log(`Partner created - Email: ${email},Password: ${temporaryPassword}`);
...
const approveTour = catchAsync(async(req,res,next) => {
    ...
    if (tour.status !== 'pending') return next(new AppError('Tour này đã được xử lý', 400));
